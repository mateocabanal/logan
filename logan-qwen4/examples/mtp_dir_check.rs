...
fn main() {
    ...
    let checks: Vec<(String, Vec<u64>)> = vec![
        ...
        (
            "hyper_connection_mixer.input_mix_weight_down.weight".into(),
            vec![hcl, hcd],
        ),
        (
            "hyper_connection_mixer.input_mix_weight_up.weight".into(),
            vec![hcd, hcl],
        ),
        (
