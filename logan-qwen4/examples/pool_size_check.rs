...
fn main() {
    ...
    let calls: Vec<_> = (0..10).map(|e| logan_qwen4::pool::ExpertCall { layer: 0, expert: e, input: inp.clone() }).collect();
    let body = logan_qwen4::pool::encode_batch_request(&calls, "qwen38-flash-next",
        Some("2a5fdfa55b5df665254c43d306f9d2839796fa1b89ff686b6cb374a75d99b15a"), d, m, "silu");
    println!("  request body: {} KB for 10 experts", body.len()/1024);
    println!("  -> {} KB per token at 48 layers", 48*body.len()/1024/1024);
    println!();
