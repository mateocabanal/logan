...
impl Model {
    ...
    /// Zero every sequence-scoped state buffer so a new conversation cannot
    /// observe the previous one's recurrent history.
    ///
    /// The fixture caught nothing here because it is pure-KV (position-indexed,
    /// overwritten). Cyber-Tiel is a hybrid: its GDN (linear-attention) layers
    /// carry recurrent/conv state that is NOT position-addressed, so a second
    /// session decoded from the first session's hidden state and produced
    /// plausible wrong tokens (observed: prompt-echo). The field set is exactly
    /// the MTP-boundary restore's: GDN recurrent/conv, PLE, HC handoff. KV/QSA
    /// are position-indexed and need no clearing; weights are never touched.
    pub fn reset_sequence_state(&mut self) {
        for li in 0..self.cfg.layers {
            self.gdn_s[li].fill(0.0);
            self.gdn_conv[li].fill(0.0);
            // Metal GDN mirrors live in their own page-aligned allocs; zero
            // them too or the GPU path would resurrect the stale state.
            if let Some(gm) = self.gdn_metal[li].as_ref() {
                let state_len = self.cfg.lin_v_heads * self.cfg.lin_k_dim * self.cfg.lin_v_dim;
                let cdim = self.cfg.lin_k_dim * self.cfg.lin_k_heads * 2
                    + self.cfg.lin_v_dim * self.cfg.lin_v_heads;
                let conv_len = cdim * self.cfg.conv_kernel.saturating_sub(1);
                unsafe {
                    std::ptr::write_bytes(gm.state, 0, state_len);
                    std::ptr::write_bytes(gm.conv_state, 0, conv_len);
                }
            }
        }
    ...
    pub fn prefill_chunk(
        ...
        let c = self.cfg.clone();
        if c.hc_count == 0 {
            // Layer-major prefill for the classic Qwen3.x layout.
            //
            // This used to loop token-major, which cost one expert dispatch per
            // layer PER PROMPT TOKEN. Measured on Cyber-Tiel with a real prompt:
            // prefill cost ~0.95 s per prompt token -- the same as decode -- and
            // 200 expert jobs were dispatched for a 5-token prompt + 1 new token
            // (5 rows x 40 layers). A layer-major traversal makes the chunk's
            // MoE seam one dispatch per LAYER, carrying every row's routed
            // experts, so prefill stops scaling with prompt length x layers.
            //
            // Correctness is preserved by construction: only the TRAVERSAL ORDER
            // changes. Every causal layer still sees its rows in increasing
            // position, so KV, GDN recurrent/conv state and QSA state advance in
            // exactly the canonical order; each row's stream is carried across
            // layers in `streams[row]` instead of in one local. The MoE seam
            // groups occurrences by row/rank, dispatches once, then scatters the
            // answers back to their slots so the per-row weighted reduction keeps
            // the canonical top-k order.
            //
            // Logits for the final row are the one thing that cannot be batched:
            // the tail (final HC/norm + vocabulary projection) is computed for
            // the last row only, exactly as the token-major loop did.
            let rows = tokens.len();
            let d = c.hidden;
            let mut streams: Vec<Vec<f32>> = tokens
                ...
                .collect();
            let ropes: Vec<Vec<(f32, f32)>> = (0..rows)
            ...
            for l in 0..c.layers {
                ...
                // PLE advances its ring immediately before the rows reach the PLE
                // layer, in chronological order -- only PLE consumes the ring.
                let mut mixed_rows: Vec<Vec<f32>> = Vec::with_capacity(rows);
                for row in 0..rows {
                    if c.ple_layer == l as i64 {
                        self.push_ple_ring(tokens[row] as usize);
                        self.ple_forward(&mut streams[row]);
                    }
                    let mut mixed = vec![0.0; d];
                    if c.zero_centered_norm {
                        rmsnorm_row(&mut mixed, &streams[row], &layer.in_ln, c.eps);
                    } else {
                        rmsnorm_row_shifted(&mut mixed, &streams[row], &layer.in_ln, c.eps);
                    }
                    mixed_rows.push(mixed);
                }

                // Temporal phase, rows in increasing position.
                let mut attn_rows: Vec<Vec<f32>> = vec![vec![0.0; d]; rows];
                for row in 0..rows {
                    let pos = start_pos + row;
                    if layer.is_gdn {
                        let mut _gdn_t = logan_core::telemetry::Span::begin("gdn");
                        self.gdn_token(&mut layer, l, &mixed_rows[row], &mut attn_rows[row]);
                ...
                for row in 0..rows {
                    for dd in 0..d {
                        streams[row][dd] += attn_rows[row][dd];
                    }
                }

                // MoE seam: every row's routed experts for THIS layer in one
                // dispatch.
                let mut moe_inputs: Vec<Vec<f32>> = Vec::with_capacity(rows);
                for row in 0..rows {
                    let mut m2 = vec![0.0; d];
                    if c.zero_centered_norm {
                        rmsnorm_row(&mut m2, &streams[row], &layer.hc_mlp_norm, c.eps);
                    } else {
                        rmsnorm_row_shifted(&mut m2, &streams[row], &layer.hc_mlp_norm, c.eps);
                    }
                    moe_inputs.push(m2);
                }
                ...
                let delegated = self.expert_source.is_some() || self.pool.is_some();
                let mut moe_rows = vec![vec![0.0f32; c.hidden]; rows];
                let mut pooled = false;
                if delegated && l < c.layers {
                    ...
                    if pool_trace_enabled() {
                        eprintln!(
                            "[pool-trace] layer {l} PREFILL-BATCHED items={} ({rows} tokens)",
                            calls.len()
                        );
                    }
                    let mut _fill_t = logan_core::telemetry::Span::begin("fill");
                ...
                // Shared expert is always local, whether or not routing was
                // delegated.
                for row in 0..rows {
                    if !pooled {
                        let mut moe = vec![0.0; d];
                        self.moe_token(&layer, l, &moe_inputs[row], &mut moe);
                        moe_rows[row] = moe;
                    }
                    let sy_gs = self.shared_expert_value(&layer, l, &moe_inputs[row]);
                    for dd in 0..d {
                        moe_rows[row][dd] += sy_gs.0[dd] * sy_gs.1;
                    }
                    for dd in 0..d {
                        streams[row][dd] += moe_rows[row][dd];
                    }
                }
...
mod tests {
    ...
    #[test]
    fn qwen_mlx_affine_matmul_uses_cuda_when_enabled() {
        const CHILD: &str = "LOGAN_MLX_AFFINE_CUDA_CHILD";
        if std::env::var_os(CHILD).is_some() {
            if !logan_core::cuda::available() {
                println!("SKIP mlx-affine CUDA: {:?}", logan_core::cuda::init_error());
                return;
            }
            const O: usize = 7;
            const I: usize = 128;
            let x: Vec<f32> = (0..I).map(|c| ((c as f32) * 0.03125).sin() * 0.5).collect();

            let pack = |bits: u8| {
                let row_words = I * bits as usize / 32;
                let mut words = vec![0_u32; O * row_words];
                let mask = (1_u32 << bits) - 1;
                for row in 0..O {
                    for col in 0..I {
                        let code = ((row * 11 + col * 7 + 3) as u32) & mask;
                        let bit = col * bits as usize;
                        let wi = row * row_words + bit / 32;
                        let shift = bit % 32;
                        words[wi] |= code << shift;
                        if shift + bits as usize > 32 {
                            words[wi + 1] |= code >> (32 - shift);
                        }
                    }
                }
                words
            ...
            for (bits, group) in [(4_u8, 64_usize), (5, 128), (6, 64), (8, 128)] {
                let weights = pack(bits);
                let groups = I / group;
                let scales = (0..O * groups)
                    .flat_map(|g| bf16(0.03125 + (g % 3) as f32 * 0.015625))
                    .collect::<Vec<_>>();
                let biases = (0..O * groups)
                    .flat_map(|g| bf16(-0.125 + (g % 5) as f32 * 0.03125))
                    .collect::<Vec<_>>();
                ...
                let before_launch = logan_core::cuda::launches();
                let before_upload = super::mlx_affine_cuda::weight_uploads();
                for _ in 0..2 {
                ...
                assert_eq!(
                    ...
                    2,
                    "two affine GEMVs must be two CUDA launches for bits={bits}"
                );
                assert_eq!(
                    super::mlx_affine_cuda::weight_uploads() - before_upload,
                    1,
                    "packed weights must upload once and stay resident for bits={bits}"
                );
                println!("mlx-affine cuda bits={bits} group={group} launches=2 uploads=1");
            }
            return;
        }
