...
mod tests {
    ...
    fn scheduled_prompt_tail_is_consumed_exactly_once() {
        ...
        let mut bridge = ResidencyBridge::new(&plan).unwrap();
        let output =
            run_session_inner(&handle, session, &mut bridge, &completed, &[5, 7, 9], 2).unwrap();
    ...
    fn layer_major_prefill_matches_token_major_tiny_fixture() {
        ...
        let canonical_logits =
            token_major.forward_token(*prompt.last().unwrap() as usize, prompt.len() - 1);
        ...
        assert_eq!(
            layer_logits, canonical_logits,
            "layer-major prefill changed final logits"
        );
