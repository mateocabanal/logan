...
pub fn recompile(request: &RecompileRequest) -> Result<RecompileSummary> {
    ...
    let selected_optimizer = if request.optimize {
        ...
        Some(select_plan(&plans, selector).cloned().ok_or_else(|| {
            ColicError::Usage(format!(
    ...
    if request.source == request.output {
        ...
        // and rewrite only selected records. Preflight the complete layout
        // before mutating any package bytes.
...
fn fixed_resident_bytes(record: &RecordInfo) -> u64 {
    ...
    if record.kind == 1
        && record
            .name
            .as_deref()
            .is_some_and(|name| name.contains("ple.ple_embedding.ngram_embedding.shard_"))
    {
        0
    } else {
        record.decoded
...
fn plan_record(
    ...
    plan_record_with_quant(package, record, request, target_profile, target_kind, quant)
}
...
mod tests {
    ...
    fn streamed_ple_ngram_shards_are_not_fixed_resident() {
        let streamed = RecordInfo {
            ...
            shard_id: 0,
            name: Some(
                "model.language_model.layers.0.ple.ple_embedding.ngram_embedding.shard_17".into(),
            ),
            layer: 0,
            expert: -1,
            offset: 0,
        ...
        ngram_scale.name = Some(
            "model.language_model.layers.0.ple.ple_embedding.ngram_embedding.weight_scale".into(),
        );
