...
fn header_extent(header: &str) -> Option<(u64, usize)> {
    ...
    while let Some(at) = rest.find("\"data_offsets\":[") {
        ...
        let close = rest.find(']')?;
        let end = rest[..close].split(',').nth(1)?.trim().parse::<u64>().ok()?;
...
fn verify_exported_layer(
    ...
    let parsed = parsed.as_object().ok_or_else(|| ColicError::InvalidSource {
        path: path.to_path_buf(),
        ...
        for (kind, elem) in [("weight", 4_u64), ("scales", 1_u64)] {
            ...
            if len == 0 || len % elem != 0 {
                return Err(ColicError::InvalidSource {
                    path: path.to_path_buf(),
                    detail: format!("tensor `{name}` declares {len} bytes, not a multiple of {elem}"),
                });
            }
...
pub fn export_experts(
    ...
    let first_layer = if resume {
        ...
        progress(done, total_experts, complete.min(plan.layers.saturating_sub(1)));
        complete
    } else {
        0
    };
    ...
    for layer in first_layer..plan.layers {
        ...
        let (gate, gate_dtypes) = pack_projection(
            &plan,
            &inventory,
            source_root,
            layer,
