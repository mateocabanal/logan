...
use logan_ir::{Placement, PlanArtifact};
...
fn bf16_qwen4_compiles_to_verified_apple8_with_mxfp4_expert_and_f8_ple() {
    ...
    let plan_path = root.join("qwen4-apple8.plan");
    ...
    request.plan = Some(plan_path.clone());
    request.verify = true;
    compile(&request, &mut NoProgress).unwrap();

    let plan = PlanArtifact::from_bytes(&fs::read(&plan_path).unwrap()).unwrap();
    let shard_name = "layers.0.ple.ple_embedding.ngram_embedding.shard_000";
    assert_eq!(
        plan.memory
            .placement
            .iter()
            .find(|(name, _)| name == shard_name)
            .map(|(_, placement)| *placement),
        Some(Placement::Streamed)
    );
    assert_eq!(
        plan.memory
            .quant
    ...
    let shard = package.record_by_name(shard_name).unwrap();
    ...
    assert_eq!(
        package.read_tensor_payload(scale).unwrap(),
        vec![0x80, 0x3f]
    ); // BF16 1.0
