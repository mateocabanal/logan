...
extern "C" int coli_bnns_f16_matmul(const uint16_t *w,
                                     const uint16_t *x,
                                     float *y,
                                     int O,
                                     int I) {
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    const size_t need = BNNSMatMulWorkspaceSize(false, true, 1.0f, &a, &b, &c, nullptr);
    static thread_local std::vector<uint8_t> workspace_f16;
    if (workspace_f16.size() < need) workspace_f16.resize(need);
    void *ws = need ? workspace_f16.data() : nullptr;
    const int rc = BNNSMatMul(false, true, 1.0f, &a, &b, &c, ws, nullptr);
#pragma clang diagnostic pop
    return rc == 0 ? 1 : 0;
}
