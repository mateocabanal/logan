...
inline float spark_qmv_bf16(float x) {
...
#define SPARK_QMV_ROW(R,ACC) do { if(row0+(R)<O){ \
      ...
      float qd=spark_qmv_bf16(sc*float(q[0])+bi)*x0+ \
               spark_qmv_bf16(sc*float(q[1])+bi)*x1+ \
               spark_qmv_bf16(sc*float(q[2])+bi)*x2+ \
               spark_qmv_bf16(sc*float(q[3])+bi)*x3+ \
               spark_qmv_bf16(sc*float(q[4])+bi)*x4+ \
               spark_qmv_bf16(sc*float(q[5])+bi)*x5+ \
               spark_qmv_bf16(sc*float(q[6])+bi)*x6+ \
               spark_qmv_bf16(sc*float(q[7])+bi)*x7; \
      (ACC)+=qd; }} while(0)
...
// Standard Llama/MiniCPM5 decode kernels. Unlike the Spark-X2.5 path above,
// these keep the residual/KV stream in fp32 so the fused path preserves the
// existing DenseSession arithmetic contract.
kernel void llama_qkv_rope(device float* qkv [[buffer(0)]],
                           constant int& H [[buffer(1)]],
                           constant int& KH [[buffer(2)]],
                           constant int& hd [[buffer(3)]],
                           constant int& rd [[buffer(4)]],
  ...
  int qdim = H * hd;
  if (int(gid) < qtasks) {
    int h = int(gid) / rh, j = int(gid) % rh;
    device float* v = qkv + h * hd;
    float inv = pow(theta, -2.0f * float(j) / float(rd));
    float ang = float(pos) * inv, cs = cos(ang), sn = sin(ang);
    float a = v[j], b = v[j + rh];
    v[j] = a * cs - b * sn;
    v[j + rh] = a * sn + b * cs;
  } else if (int(gid) - qtasks < ktasks) {
    int z = int(gid) - qtasks, h = z / rh, j = z % rh;
    device float* v = qkv + qdim + h * hd;
    float inv = pow(theta, -2.0f * float(j) / float(rd));
    float ang = float(pos) * inv, cs = cos(ang), sn = sin(ang);
    float a = v[j], b = v[j + rh];
    v[j] = a * cs - b * sn;
...
kernel void llama_cache_store(device const float* qkv [[buffer(0)]],
                              device float* kc [[buffer(1)]],
                              device float* vc [[buffer(2)]],
                              constant int& qdim [[buffer(3)]],
                              constant int& kvdim [[buffer(4)]],
                              constant int& slot [[buffer(5)]],
                              uint i [[thread_position_in_grid]]) {
  if (i < uint(kvdim)) {
    kc[(long)slot * kvdim + i] = qkv[qdim + i];
    vc[(long)slot * kvdim + i] = qkv[qdim + kvdim + i];
  }
}
kernel void llama_attn_score(device const float* qkv [[buffer(0)]],
                             device const float* kc [[buffer(1)]],
                             device float* score [[buffer(2)]],
                             constant int& H [[buffer(3)]],
                             constant int& KH [[buffer(4)]],
                             constant int& hd [[buffer(5)]],
                             constant int& kvdim [[buffer(6)]],
                             constant int& T [[buffer(7)]],
                             uint tg [[threadgroup_position_in_grid]],
                             uint lane [[thread_index_in_simdgroup]]) {
  int h = int(tg) / T, ti = int(tg) % T;
  if (h >= H) return;
  int rep = H / KH, kh = h / rep;
  device const float* q = qkv + h * hd;
  device const float* k = kc + (long)ti * kvdim + kh * hd;
  float acc = 0.0f;
  for (int d = int(lane); d < hd; d += 32) acc += q[d] * k[d];
  acc = simd_sum(acc);
  if (lane == 0) score[(long)h * T + ti] = acc * rsqrt(float(hd));
}
kernel void llama_attn_ctx(device const float* score [[buffer(0)]],
                           ...
                           constant int& T [[buffer(7)]],
                           uint gid [[thread_position_in_grid]]) {
  int h = int(gid) / hd, d = int(gid) % hd;
  if (h >= H) return;
  int kh = h / (H / KH);
  float acc = 0.0f;
  for (int ti = 0; ti < T; ++ti)
    acc += score[(long)h * T + ti] * vc[(long)ti * kvdim + kh * hd + d];
  out[gid] = acc;
}
...
static bool ensure_multi_buffer(id<MTLBuffer> __strong *b, size_t *cap, size_t need) {
  if (*b && *cap >= need) return true;
  id<MTLBuffer> replacement = [g_dev newBufferWithLength:need options:g_res_opts];
  ...
  @autoreleasepool {
    ...
    for (int di = 0; di < count; ++di) {
      ...
      if (!ensure_multi_buffer(&g_multi_outs[(size_t)di], &g_multi_out_caps[(size_t)di],
                               out_elems * sizeof(float))) return 0;
    }
...
// ---- Standard Llama/MiniCPM5 one-command-buffer decode ---------------------
struct LlamaLayerCtx {
  uint64_t model_id = 0;
  int layer = -1, D = 0, inter = 0, H = 0, KH = 0, hd = 0;
  int tokens = 0, cache_cap = 0;
  float theta = 0.0f, eps = 0.0f;
  const float *host_in_norm = nullptr, *host_post_norm = nullptr;
  id<MTLBuffer> in_norm = nil, post_norm = nil;
  id<MTLBuffer> x = nil, xn = nil, qkv = nil, att = nil, ao = nil;
  id<MTLBuffer> pn = nil, mg = nil, mu = nil, mo = nil;
  id<MTLBuffer> kc = nil, vc = nil, score = nil;
};
static std::vector<LlamaLayerCtx *> g_llama_layers;

static bool llama_resize_cache(LlamaLayerCtx *c, int need) {
  if (!c || need <= c->cache_cap) return true;
  int cap = ((need + 255) / 256) * 256;
  size_t kv = (size_t)c->KH * (size_t)c->hd;
  id<MTLBuffer> nk = [g_dev newBufferWithLength:(size_t)cap * kv * sizeof(float)
                                         options:g_res_opts];
  id<MTLBuffer> nv = [g_dev newBufferWithLength:(size_t)cap * kv * sizeof(float)
                                         options:g_res_opts];
  id<MTLBuffer> ns = [g_dev newBufferWithLength:(size_t)cap * (size_t)c->H * sizeof(float)
                                         options:g_res_opts];
  if (!nk || !nv || !ns) return false;
  if (c->kc && c->cache_cap > 0) {
    size_t old = (size_t)c->cache_cap * kv * sizeof(float);
    memcpy(nk.contents, c->kc.contents, old);
    memcpy(nv.contents, c->vc.contents, old);
  }
  c->kc = nk; c->vc = nv; c->score = ns; c->cache_cap = cap;
  return true;
}

static LlamaLayerCtx *llama_ctx(uint64_t model_id, int layer,
                                const float *in_norm, const float *post_norm,
                                int D, int inter, int H, int KH, int hd,
                                float theta, float eps) {
  for (LlamaLayerCtx *c : g_llama_layers) {
    if (!c || c->model_id != model_id || c->layer != layer) continue;
    if (c->host_in_norm != in_norm || c->host_post_norm != post_norm ||
        c->D != D || c->inter != inter || c->H != H || c->KH != KH ||
        c->hd != hd || c->theta != theta || c->eps != eps)
      return nullptr;
    return c;
  }
  if (!model_id || layer < 0 || !in_norm || !post_norm || D <= 0 || inter <= 0 ||
      H <= 0 || KH <= 0 || H % KH != 0 || hd <= 0 || hd > 1024 ||
      !(theta > 0.0f) || !(eps > 0.0f))
    return nullptr;
  auto *c = new (std::nothrow) LlamaLayerCtx();
  if (!c) return nullptr;
  c->model_id = model_id; c->layer = layer; c->D = D; c->inter = inter;
  c->H = H; c->KH = KH; c->hd = hd; c->theta = theta; c->eps = eps;
  c->host_in_norm = in_norm; c->host_post_norm = post_norm;
  c->in_norm = [g_dev newBufferWithBytes:in_norm length:(size_t)D * sizeof(float)
                                 options:g_res_opts];
  c->post_norm = [g_dev newBufferWithBytes:post_norm length:(size_t)D * sizeof(float)
                                   options:g_res_opts];
  int qdim = H * hd, kvdim = KH * hd;
  auto B = [&](size_t n) { return [g_dev newBufferWithLength:n options:g_res_opts]; };
  c->x = B((size_t)D * sizeof(float));
  c->xn = B((size_t)D * sizeof(float));
  c->qkv = B((size_t)(qdim + 2 * kvdim) * sizeof(float));
  c->att = B((size_t)qdim * sizeof(float));
  c->ao = B((size_t)D * sizeof(float));
  c->pn = B((size_t)D * sizeof(float));
  c->mg = B((size_t)inter * sizeof(float));
  c->mu = B((size_t)inter * sizeof(float));
  c->mo = B((size_t)D * sizeof(float));
  if (!c->in_norm || !c->post_norm || !c->x || !c->xn || !c->qkv ||
      !c->att || !c->ao || !c->pn || !c->mg || !c->mu || !c->mo ||
      !llama_resize_cache(c, 1)) {
    delete c;
    return nullptr;
  }
  g_llama_layers.push_back(c);
  return c;
}

static ColiMetalTensor *llama_tensor(ColiMetalMatmulDesc &d) {
  if (!d.weights || d.I <= 0 || d.O <= 0 || (d.fmt != 5 && d.fmt != 15))
    return nullptr;
  if (d.tensor) {
    if (d.tensor->fmt != d.fmt || d.tensor->I != d.I || d.tensor->O != d.O)
      return nullptr;
    return d.tensor;
  }
  auto *t = new (std::nothrow) ColiMetalTensor();
  if (!t) return nullptr;
  t->fmt = d.fmt; t->I = d.I; t->O = d.O; t->gs = d.gs;
  t->wbytes = fmt_bytes(d.fmt, d.I, d.O);
  t->w = wrap(d.weights, t->wbytes);
  t->s = d.fmt == 15
      ? wrap(d.scales, fmt_scale_bytes(d.fmt, d.I, d.O, d.gs))
      : nil;
  t->woff = t->soff = 0;
  if (!t->w || (d.fmt == 15 && !t->s)) { delete t; return nullptr; }
  d.tensor = t;
  g_tensor_count++; g_tensor_bytes += t->wbytes;
  return t;
}

static bool llama_gemv(id<MTLComputeCommandEncoder> e, ColiMetalTensor *t,
                       id<MTLBuffer> x, id<MTLBuffer> y, size_t yoff,
                       int I, int O) {
  if (!e || !t || (t->fmt != 5 && t->fmt != 15) || t->I != I || t->O != O)
    return false;
  if (t->fmt == 15) {
    [e setComputePipelineState:g_sp_qmv];
    [e setBuffer:t->w offset:t->woff atIndex:0];
    [e setBuffer:t->s offset:t->soff atIndex:1];
    [e setBuffer:x offset:0 atIndex:2];
    [e setBuffer:y offset:yoff atIndex:3];
    [e setBytes:&I length:4 atIndex:4]; [e setBytes:&O length:4 atIndex:5];
    [e dispatchThreadgroups:MTLSizeMake(((size_t)O + 7) / 8, 1, 1)
  ...
  [e setBuffer:x offset:0 atIndex:2];
  [e setBuffer:y offset:yoff atIndex:3];
  int S = 1, fmt = 5, NT = O, gs = 0;
  [e setBytes:&S length:4 atIndex:4]; [e setBytes:&I length:4 atIndex:5];
  [e setBytes:&O length:4 atIndex:6]; [e setBytes:&fmt length:4 atIndex:7];
  [e setBytes:&NT length:4 atIndex:8]; [e setBytes:&gs length:4 atIndex:9];
  [e dispatchThreadgroups:MTLSizeMake(((size_t)O + 3) / 4, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(128, 1, 1)];
  return true;
}

static bool llama_prepare_layer(uint64_t model_id, int layer,
                                ColiMetalMatmulDesc *descs, int count,
                                const float *in_norm, const float *post_norm,
                                int D, int inter, int H, int KH, int hd,
                                int pos, float theta, float eps,
                                LlamaLayerCtx **out, ColiMetalTensor **wt) {
  if (!descs || count != 7 || !out || !wt || pos < 0) return false;
  int qdim = H * hd, kvdim = KH * hd;
  const int expected_i[7] = {D, D, D, qdim, D, D, inter};
  const int expected_o[7] = {qdim, kvdim, kvdim, D, inter, inter, D};
  for (int i = 0; i < 7; ++i)
    if ((descs[i].fmt != 5 && descs[i].fmt != 15) ||
        descs[i].I != expected_i[i] || descs[i].O != expected_o[i])
      return false;
  LlamaLayerCtx *c = llama_ctx(model_id, layer, in_norm, post_norm,
                               D, inter, H, KH, hd, theta, eps);
  if (!c || c->tokens != pos || !llama_resize_cache(c, pos + 1)) return false;
  for (int i = 0; i < 7; ++i) {
    wt[i] = llama_tensor(descs[i]);
...
static bool llama_encode_layer(id<MTLComputeCommandEncoder> e, LlamaLayerCtx *c,
                               ColiMetalTensor **wt, int D, int inter, int H,
                               int KH, int hd, int pos, float theta, float eps) {
  if (!e || !c || !wt) return false;
  int qdim = H * hd, kvdim = KH * hd, qkvdim = qdim + 2 * kvdim;
  int T = pos + 1, slot = pos, rd = hd;
  auto BAR = [&] { [e memoryBarrierWithScope:MTLBarrierScopeBuffers]; };
  [e setComputePipelineState:g_sp_copy];
  [e setBuffer:c->x offset:0 atIndex:0]; [e setBuffer:c->xn offset:0 atIndex:1];
  [e setBytes:&D length:4 atIndex:2];
  [e dispatchThreads:MTLSizeMake(D, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_a_rms];
  [e setBuffer:c->xn offset:0 atIndex:0]; [e setBuffer:c->in_norm offset:0 atIndex:1];
  ...
  BAR();
  if (!llama_gemv(e, wt[0], c->xn, c->qkv, 0, D, qdim)) return false;
  if (!llama_gemv(e, wt[1], c->xn, c->qkv, (size_t)qdim * sizeof(float), D, kvdim)) return false;
  if (!llama_gemv(e, wt[2], c->xn, c->qkv, (size_t)(qdim + kvdim) * sizeof(float), D, kvdim)) return false;
  BAR();
  [e setComputePipelineState:g_llama_rope]; [e setBuffer:c->qkv offset:0 atIndex:0];
  [e setBytes:&H length:4 atIndex:1]; [e setBytes:&KH length:4 atIndex:2];
  [e setBytes:&hd length:4 atIndex:3]; [e setBytes:&rd length:4 atIndex:4];
  [e setBytes:&pos length:4 atIndex:5]; [e setBytes:&theta length:4 atIndex:6];
  int rope_tasks = (H + KH) * (rd / 2);
  [e dispatchThreads:MTLSizeMake((size_t)rope_tasks, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_llama_store];
  [e setBuffer:c->qkv offset:0 atIndex:0]; [e setBuffer:c->kc offset:0 atIndex:1];
  [e setBuffer:c->vc offset:0 atIndex:2]; [e setBytes:&qdim length:4 atIndex:3];
  [e setBytes:&kvdim length:4 atIndex:4]; [e setBytes:&slot length:4 atIndex:5];
  [e dispatchThreads:MTLSizeMake((size_t)kvdim, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_llama_score];
  [e setBuffer:c->qkv offset:0 atIndex:0]; [e setBuffer:c->kc offset:0 atIndex:1];
  [e setBuffer:c->score offset:0 atIndex:2]; [e setBytes:&H length:4 atIndex:3];
  [e setBytes:&KH length:4 atIndex:4]; [e setBytes:&hd length:4 atIndex:5];
  [e setBytes:&kvdim length:4 atIndex:6]; [e setBytes:&T length:4 atIndex:7];
  [e dispatchThreadgroups:MTLSizeMake((size_t)H * T, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(32, 1, 1)];
  BAR();
  [e setComputePipelineState:g_sp_softmax]; [e setBuffer:c->score offset:0 atIndex:0];
  [e setBytes:&T length:4 atIndex:1];
  [e dispatchThreadgroups:MTLSizeMake((size_t)H, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_llama_ctx]; [e setBuffer:c->score offset:0 atIndex:0];
  [e setBuffer:c->vc offset:0 atIndex:1]; [e setBuffer:c->att offset:0 atIndex:2];
  [e setBytes:&H length:4 atIndex:3]; [e setBytes:&KH length:4 atIndex:4];
  [e setBytes:&hd length:4 atIndex:5]; [e setBytes:&kvdim length:4 atIndex:6];
  [e setBytes:&T length:4 atIndex:7];
  [e dispatchThreads:MTLSizeMake((size_t)qdim, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  if (!llama_gemv(e, wt[3], c->att, c->ao, 0, qdim, D)) return false;
  BAR();
  [e setComputePipelineState:g_a_add]; [e setBuffer:c->x offset:0 atIndex:0];
  [e setBuffer:c->ao offset:0 atIndex:1];
  [e dispatchThreads:MTLSizeMake((size_t)D, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_sp_copy]; [e setBuffer:c->x offset:0 atIndex:0];
  [e setBuffer:c->pn offset:0 atIndex:1]; [e setBytes:&D length:4 atIndex:2];
  [e dispatchThreads:MTLSizeMake(D, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  [e setComputePipelineState:g_a_rms]; [e setBuffer:c->pn offset:0 atIndex:0];
  [e setBuffer:c->post_norm offset:0 atIndex:1]; [e setBytes:&D length:4 atIndex:2];
  [e setBytes:&eps length:4 atIndex:3];
  [e dispatchThreadgroups:MTLSizeMake(1, 1, 1) threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  if (!llama_gemv(e, wt[4], c->pn, c->mg, 0, D, inter)) return false;
  if (!llama_gemv(e, wt[5], c->pn, c->mu, 0, D, inter)) return false;
  BAR();
  [e setComputePipelineState:g_moe_silu]; [e setBuffer:c->mg offset:0 atIndex:0];
  [e setBuffer:c->mu offset:0 atIndex:1];
  [e dispatchThreads:MTLSizeMake((size_t)inter, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  BAR();
  if (!llama_gemv(e, wt[6], c->mg, c->mo, 0, inter, D)) return false;
  BAR();
  [e setComputePipelineState:g_a_add]; [e setBuffer:c->x offset:0 atIndex:0];
  [e setBuffer:c->mo offset:0 atIndex:1];
  [e dispatchThreads:MTLSizeMake((size_t)D, 1, 1)
        threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
  (void)qkvdim;
  return true;
...
extern "C" int coli_metal_llama_layer(
    uint64_t model_id, int layer, ColiMetalMatmulDesc *descs, int count,
    float *x, float *k_out, float *v_out, const float *in_norm,
    const float *post_norm, int D, int inter, int H, int KH, int hd, int pos,
    float theta, float eps) {
  if (!g_dev || !g_queue || !x || !k_out || !v_out || !in_norm || !post_norm)
    return 0;
  std::lock_guard<std::mutex> lk(g_op_mtx);
  @autoreleasepool {
    LlamaLayerCtx *c = nullptr; ColiMetalTensor *wt[7] = {};
    if (!llama_prepare_layer(model_id, layer, descs, count, in_norm, post_norm,
                             D, inter, H, KH, hd, pos, theta, eps, &c, wt))
      return 0;
    memcpy(c->x.contents, x, (size_t)D * sizeof(float));
    uint64_t t0 = g_coli_metal_profile_on ? mnow_ns() : 0;
    id<MTLCommandBuffer> cb = [g_queue commandBuffer];
    id<MTLComputeCommandEncoder> e = [cb computeCommandEncoder];
    if (!cb || !e || !llama_encode_layer(e, c, wt, D, inter, H, KH, hd, pos, theta, eps))
      return 0;
    [e endEncoding];
    if (t0) { uint64_t t1 = mnow_ns(); g_metal_prof.encode_ns += t1 - t0; t0 = t1; }
    [cb commit];
    ...
    memcpy(x, c->x.contents, (size_t)D * sizeof(float));
    int qdim = H * hd, kvdim = KH * hd;
    memcpy(k_out, (char *)c->qkv.contents + (size_t)qdim * sizeof(float),
           (size_t)kvdim * sizeof(float));
    memcpy(v_out, (char *)c->qkv.contents + (size_t)(qdim + kvdim) * sizeof(float),
           (size_t)kvdim * sizeof(float));
    c->tokens = pos + 1;
  }
  return 1;
...
extern "C" void coli_metal_llama_drop_model(uint64_t model_id) {
  if (!model_id) return;
  std::lock_guard<std::mutex> lk(g_op_mtx);
  for (auto it = g_llama_layers.begin(); it != g_llama_layers.end();) {
    LlamaLayerCtx *c = *it;
    if (c && c->model_id == model_id) {
      delete c; it = g_llama_layers.erase(it);
    } else ++it;
  }
}
