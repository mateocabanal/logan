 ...
 *  - one MTLIOCommandQueue + one MTLSharedEvent for load tickets; exact CPU
 *    completion is tracked by each load's MTLIOCommandBuffer because the IO
 *    queue is concurrent and shared-event values can complete out of order
...
/* A/B gate for retaining freed MTLBuffers as a physical slot pool. */
static _Atomic int g_reuse_slots = 0;
...
/* Separate queue-barrier event. Unlike per-load g_ev values, each signal is
 * ordered after enqueueBarrier(), so observing it proves every I/O command
 * committed before the barrier has completed even on a concurrent IO queue. */
static id<MTLSharedEvent> g_batch_ev;
...
/* persistent slots: [slot id] -> buffer + size + exact last-load command.
 * Ids are REUSABLE: freed ids return to the pool, active set stays bounded.
 *
 * The shared-event value is an opaque public ticket only. MetalIO queues are
 * concurrent, so a later command buffer can signal a larger shared-event
 * value before an earlier command has completed. Exact CPU completion must
 * therefore be tied to the MTLIOCommandBuffer itself, never inferred from
 * the event watermark. */
#define METALIO_MAX_SLOTS 4096
static struct {
    id<MTLBuffer> buf;
    id<MTLIOCommandBuffer> last_cb; /* exact command backing last_event */
    size_t bytes;
    int in_use;
...
int metalio_init(void){
    ...
    if (@available(macOS 13.0, *)) {
        ...
        g_batch_ev = [g_dev newSharedEvent];
        if (!g_ev || !g_batch_ev) return 0;
...
void metalio_shutdown(void){
    ...
    [g_lock lock];
    /* A concurrent MTLIO queue may complete command N+1 before N, so a
     * shared-event watermark cannot prove the queue is drained. Wait every
     * slot's exact command before releasing buffers/file handles. */
    for (int i = 0; i < g_nslots; i++) {
        id<MTLIOCommandBuffer> cb = g_slots[i].last_cb;
        if (cb && cb.status == MTLIOStatusPending) [cb waitUntilCompleted];
        g_slots[i].last_cb = nil;
    }
    verbose("shutdown: drained exact MetalIO commands");
    for (int i = 0; i < g_nfiles; i++) g_files[i] = nil;
    g_nfiles = 0;
    for (int i = 0; i < g_nslots; i++) { g_slots[i].buf = nil; g_slots[i].bytes = 0; }
    g_nslots = 0;
    g_iq = nil; g_ev = nil; g_batch_ev = nil; g_dev = nil;
    [g_lock unlock];
...
int metalio_slot_alloc(size_t max_bytes){
    ...
    if (@available(macOS 13.0, *)) {
        ...
        if (sid >= 0) {
            ...
            const size_t old_bytes = g_slots[sid].bytes;
            if (!b || old_bytes < len) {
                b = [g_dev newBufferWithLength:len options:MTLResourceStorageModeShared];
                if (b) {
                    g_slots[sid].buf = b;
...
void metalio_slot_free(int slot){
    ...
    if (cb && cb.status == MTLIOStatusPending) [cb waitUntilCompleted];
    int64_t last_ev = atomic_load_explicit(&g_slots[slot].last_event, memory_order_relaxed);
    int64_t consumed = atomic_load_explicit(&g_slots[slot].consumed, memory_order_relaxed);
    if (g_slots[slot].last_kind == MIO_LOAD_SPEC && last_ev > consumed)
...
int64_t metalio_loadv(int slot, const ColiMetalioRegion *regions, int count,
    ...
    if (@available(macOS 13.0, *)) {
        if (regions_ok(slot, regions, count)) {
            id<MTLIOCommandBuffer> previous = g_slots[slot].last_cb;
            if (previous && previous.status == MTLIOStatusPending) {
                /* Concurrent writes into one execution slot are never safe:
                 * reject rather than overwrite the exact completion handle. */
            ...
            [ioCB addCompletedHandler:^(id<MTLIOCommandBuffer> completed) {
                atomic_fetch_sub_explicit(&m_outstanding, 1, memory_order_relaxed);
...
int64_t metalio_batch_barrier(void){
    if (!atomic_load_explicit(&g_active, memory_order_relaxed) || !g_iq || !g_batch_ev)
        return -1;
    [g_lock lock];
    int64_t out = -1;
    if (@available(macOS 13.0, *)) {
        /* Apple guarantees that enqueueBarrier() finishes every in-flight IO
         * command buffer before any new one runs. The signal-only command
         * therefore represents exact completion of the whole issued batch. */
        [g_iq enqueueBarrier];
        id<MTLIOCommandBuffer> ioCB = [g_iq commandBuffer];
        if (ioCB) {
            uint64_t v = g_batch_ev_val++;
            [ioCB signalEvent:g_batch_ev value:v];
            [ioCB commit];
            out = (int64_t)v;
            verbose("batch_barrier: event=%llu", (unsigned long long)v);
        }
    }
    [g_lock unlock];
    return out;
...
int metalio_batch_wait(int64_t event_value, const int *slots, int count){
    if (!atomic_load_explicit(&g_active, memory_order_relaxed) || event_value <= 0 ||
        !g_batch_ev || count < 0 || (count > 0 && !slots)) return -1;
    [g_lock lock];
    const int valid = event_value <= (int64_t)g_batch_ev_val - 1;
    id<MTLSharedEvent> ev = g_batch_ev;
    [g_lock unlock];
    if (!valid || !ev) return -1;
    uint64_t t0 = mach_absolute_time();
    [ev waitUntilSignaledValue:(uint64_t)event_value timeoutMS:UINT64_MAX];
    static mach_timebase_info_data_t tb;
    if (tb.denom == 0) mach_timebase_info(&tb);
    uint64_t us = (mach_absolute_time() - t0) * tb.numer / tb.denom / 1000;

    /* The barrier proves every earlier command is no longer pending. Check
     * the exact selected loads as well so an I/O error never gets mistaken
     * for successful residency. */
    int ok = 1;
    id<MTLIOCommandBuffer> selected[64] = {};
    if (count > 64) ok = 0;
    [g_lock lock];
    for (int i = 0; ok && i < count; i++) {
        const int slot = slots[i];
        if (slot < 0 || slot >= g_nslots || !g_slots[slot].in_use || !g_slots[slot].last_cb) {
            ok = 0;
            break;
        }
        selected[i] = g_slots[slot].last_cb;
    }
    [g_lock unlock];

    /* The barrier orders the I/O work, but on current Metal the public
     * MTLIOCommandBuffer.status can lag the shared-event signal briefly.
     * Retire each exact selected command before judging it failed. This is
     * normally a zero-duration status fence; a genuine I/O error remains a
     * hard failure and still triggers the caller's CPU correctness fallback. */
    for (int i = 0; ok && i < count; i++) {
        id<MTLIOCommandBuffer> cb = selected[i];
        if (cb.status != MTLIOStatusComplete) [cb waitUntilCompleted];
        if (cb.status != MTLIOStatusComplete) {
            ok = 0;
            atomic_fetch_add_explicit(&m_fails, 1, memory_order_relaxed);
        }
    }
    ...
    hist_add(us);
    verbose("batch_wait: event=%lld slots=%d latency=%lluus ok=%d", (long long)event_value,
            count, (unsigned long long)us, ok);
    return ok ? 0 : -1;
}
...
int metalio_wait(int64_t event_value){
    ...
    id<MTLIOCommandBuffer> cb = nil;
    for (int i = 0; i < g_nslots; i++) {
        if (!g_slots[i].in_use) continue;
        int64_t slot_event = atomic_load_explicit(&g_slots[i].last_event, memory_order_relaxed);
        if (slot_event == event_value) {
            cb = g_slots[i].last_cb;
            break;
        }
    }
    [g_lock unlock];
    ...
    uint64_t t0 = mach_absolute_time();
    /* Do not wait on g_ev here. MTLIOCommandQueueTypeConcurrent permits a
     * later command buffer to raise the shared event past this value while
     * this exact command is still pending. */
    ...
    hist_add(us);
    if (cb.status != MTLIOStatusComplete) {
        verbose("wait: event=%lld failed status=%ld", (long long)event_value, (long)cb.status);
        return -1;
    }
...
void metalio_slot_consumed(int slot){
    ...
    if (ev > cons) {
        ...
        if (g_slots[slot].last_kind == MIO_LOAD_SPEC)
