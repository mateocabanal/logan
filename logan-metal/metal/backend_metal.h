...
/* Standard Llama/MiniCPM5 decode layer. `descs` are exactly
 * [q,k,v,o,gate,up,down], with raw BF16 (fmt=5) or affine-8/group-64
 * (fmt=15) weights. The projections, RoPE, attention, residuals, RMSNorm,
 * SwiGLU, and output projection execute in one command buffer. The GPU owns
 * the per-layer KV cache and returns the current K/V rows for the host
 * checkpoint path. Returns 0 before submit, -1 after submit failure, and 1
 * on success. */
int coli_metal_llama_layer(uint64_t model_id, int layer,
                           ColiMetalMatmulDesc *descs, int count,
                           float *x, float *k_out, float *v_out,
                           const float *in_norm, const float *post_norm,
                           int D, int inter, int heads, int kv_heads, int head_dim,
                           int pos, float theta, float eps);
void coli_metal_llama_drop_model(uint64_t model_id);
