 ...
 * the copy-backed fallback. Keep enough entries for every projection in a
 * model run and decline rather than evicting if an unexpected caller exceeds
 * the bounded process-local cache. */
struct Bf16WeightCacheEntry {
    const uint16_t *w = NULL;
    int O = 0;
    int I = 0;
    id<MTLBuffer> buffer = nil;
};
static const size_t kBf16WeightCacheCapacity = 512;
static std::vector<Bf16WeightCacheEntry> g_bf16_weight_cache;
static id<MTLBuffer> g_bf16_xbuf = nil;
static id<MTLBuffer> g_bf16_ybuf = nil;
static size_t g_bf16_x_bytes = 0, g_bf16_y_bytes = 0;

static int ensure_bf16_io_buffers_locked(size_t x_bytes, size_t y_bytes) {
    id<MTLBuffer> xb = g_bf16_xbuf;
    id<MTLBuffer> yb = g_bf16_ybuf;
    size_t xb_bytes = g_bf16_x_bytes;
    size_t yb_bytes = g_bf16_y_bytes;

    /* Allocate replacements before publishing them so a failed growth leaves
     * the previous reusable buffers intact. The direct-path lock serializes
     * all users, and callers wait for each command before returning. */
    if (!xb || xb_bytes < x_bytes) {
        xb = [g_device newBufferWithLength:x_bytes
                                  options:MTLResourceStorageModeShared];
        if (!xb) return 0;
        xb_bytes = x_bytes;
    }
    if (!yb || yb_bytes < y_bytes) {
        yb = [g_device newBufferWithLength:y_bytes
                                  options:MTLResourceStorageModeShared];
        if (!yb) return 0;
        yb_bytes = y_bytes;
    }
    ...
    g_bf16_xbuf = xb;
    g_bf16_ybuf = yb;
    g_bf16_x_bytes = xb_bytes;
    g_bf16_y_bytes = yb_bytes;
    return 1;
}

static void clear_bf16_io_buffers_locked(void) {
    g_bf16_xbuf = nil;
    g_bf16_ybuf = nil;
    g_bf16_x_bytes = g_bf16_y_bytes = 0;
}
...
static void clear_locked(void) {
    ...
    clear_bf16_io_buffers_locked();
    g_bf16_weight_cache.clear();
    ...
    const uint16_t *w, const float *x, float *y, int S, int O, int I)
{
    ...
    for (const Bf16WeightCacheEntry &entry : g_bf16_weight_cache) {
        if (entry.w == w && entry.O == O && entry.I == I) {
            weight_buffer = entry.buffer;
            break;
        }
    }
    if (!weight_buffer) {
        if (g_bf16_weight_cache.size() >= kBf16WeightCacheCapacity)
            return 0;
        ...
        if (use_nocopy)
            weight_buffer = qwen_gdn_wrap_nocopy_locked(w, w_bytes, true);

        /* Preserve the generic API for unaligned callers and an exact A/B
         * baseline via QWEN_ATTN_BF16_NOCOPY=0. */
        if (!weight_buffer) {
            weight_buffer = [g_device newBufferWithBytes:w
                                                   length:w_bytes
                                                  options:MTLResourceStorageModeShared];
        }
        if (!weight_buffer) return 0;

        Bf16WeightCacheEntry entry;
        entry.w = w;
        entry.O = O;
        entry.I = I;
        entry.buffer = weight_buffer;
        try {
            g_bf16_weight_cache.push_back(entry);
    ...
    if (!qwen_gdn_mul3_size((size_t)S, (size_t)I, sizeof(float), &x_bytes) ||
        !qwen_gdn_mul3_size((size_t)S, (size_t)O, sizeof(float), &y_bytes) ||
        !ensure_bf16_io_buffers_locked(x_bytes, y_bytes))
        return 0;
    memcpy(g_bf16_xbuf.contents, x, x_bytes);
    ...
    [enc setBuffer:g_bf16_xbuf offset:0 atIndex:1];
    [enc setBuffer:g_bf16_ybuf offset:0 atIndex:2];
    [enc setBytes:&S length:sizeof(S) atIndex:3];
    ...
    profile_completed_locked(cb, encode_ns, submit_ns, wait_ns, 0, DIRECT_PROFILE_GENERAL);
    memcpy(y, g_bf16_ybuf.contents, y_bytes);
    return 1;
}
