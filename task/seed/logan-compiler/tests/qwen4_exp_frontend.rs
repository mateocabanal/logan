...
fn fixture() -> Fixture {
    ...
    let root = std::env::temp_dir().join(format!(
        "logan-qwen4-reap288-{}-{nonce}-{fixture_id}",
        std::process::id()
    ));
...
fn reap_288_is_first_class_geometry_and_expert_slices_are_bf16() {
    let f = fixture();
    assert!(Qwen4ExpFrontend::probe(&f.inventory).unwrap());
    let model = Qwen4ExpFrontend::build(&f.inventory).unwrap();
...
fn bf16_ple_is_lowered_to_streamed_f8_shards_with_derived_scale() {
    ...
    assert!(
        !model
            .resident_tensors
            .contains_key("model.language_model.layers.0.ple.ple_embedding.ngram_embedding.weight")
    );
}
