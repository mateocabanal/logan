...
impl Qwen4ExpFrontend {
    ...
    pub fn build(source: &SourceInventory) -> Result<SemanticModel> {
        ...
        for layer in 0..layers {
            ...
            for (name, tensor) in &source.tensors {
                let Some(rest) = name.strip_prefix(&lp) else {
                    continue;
                };
        ...
        Ok(SemanticModel {
            // Keep Qwen4 distinct from the legacy Qwen3.x frontend while
            // allowing it to reuse the generic BF16->MXFP4 expert lowerer.
            // Runtime dispatch remains qwen4_exp via the copied config.json.
            architecture: Architecture::Qwen4Exp,
            geometry,
            routed_experts,
            global_tensors,
    ...
    for layer in 0..geometry.layers {
        ...
        if first_per {
            for expert in 0..ecount {
                ...
                let (gate, up) =
                    if let (Some(gate_name), Some(up_name)) = (separate_gate, separate_up) {
                        ...
                        .ok_or_else(|| {
                            missing(source, format!("expert {layer}/{expert} gate_up_proj"))
                        })?;
                        let gu = source.tensors.get(&gu_name).ok_or_else(|| {
                            missing(source, format!("expert {layer}/{expert} gate_up_proj"))
                        })?;
        ...
        let gu_name = first_existing_name(
            source,
            &[
                format!("{ep}.gate_up_proj"),
                format!("{ep}.gate_up_proj.weight"),
            ],
        )
...
fn add_ple_tensors(
    ...
    let mut shards: Vec<(u64, String, TensorRef)> = source
        ...
        .filter_map(|(name, tensor)| {
            ...
            let index = suffix.trim_end_matches(".weight").parse::<u64>().ok()?;
...
fn f8_view_of_bf16(
    ...
    if tensor.len % 2 != 0 {
        return invalid(
            &source.root,
            format!("PLE shard `{name}` has odd BF16 byte length"),
        );
    }
...
fn validate_ple_bf16(source: &SourceInventory, name: &str, tensor: &TensorRef) -> Result<()> {
    if tensor.dtype != "BF16" || tensor.shape.len() != 2 {
        return invalid(
            &source.root,
            format!(
                "PLE tensor `{name}` must be rank-2 BF16, got {}/{:?}",
    ...
    if tensor.len != expected {
        return invalid(
            &source.root,
            format!(
                "PLE tensor `{name}` len {} != expected {expected}",
                tensor.len
            ),
        );
    }
...
fn slice_fused(
    ...
    let offset = tensor
        .offset
        .checked_add(offset_rows.checked_mul(row_bytes).ok_or_else(|| {
