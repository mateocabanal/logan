fn main() {
    ...
    if os != "macos" || arch != "aarch64" { return; }
    ...
    let status = std::process::Command::new("clang++")
        .args(["-x", "objective-c++", "-fobjc-arc", "-fblocks", "-O2", "-mmacosx-version-min=11.0", "-c"])
        .arg(&src).arg("-o").arg(&obj).status().expect("clang required on macOS");
    assert!(status.success(), "logan-ane async bridge failed to compile");
    let status = std::process::Command::new("ar")
        .args(["rcs"]).arg(&lib).arg(&obj).status().expect("ar required on macOS");
