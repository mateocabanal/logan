...
pub struct RuntimeFeatures {
    ...
    pub gdn_metal: bool,
    pub gdn_ane: bool,
    pub gdn_ane_fused: bool,
    pub gdn_single_copy: bool,
    pub gdn_single_encoder: bool,
    pub attn_metal: bool,
...
pub struct RuntimeStats {
    ...
    pub gdn_metal_wait_ns: u64,
    pub gdn_metal_kernel_ns: u64,
    pub gdn_metal_calls: u64,
    pub moe_metal_wait_ns: u64,
    pub moe_metal_kernel_ns: u64,
    pub moe_metal_calls: u64,
    pub fused_calls: u64,
...
impl Model {
    pub fn runtime_stats(&self) -> RuntimeStats {
        ...
        let (gdn_wait, gdn_kernel, gdn_calls, moe_wait, moe_kernel, moe_calls) =
            logan_metal::metal_profile_detail();
        ...
        RuntimeStats {
            ...
            gdn_in_proj_ms: self.spans.gdn_in_proj_ms,
            gdn_conv_ms: self.spans.gdn_conv_ms,
            gdn_prepare_ms: self.spans.gdn_prepare_ms,
            gdn_recur_ms: self.spans.gdn_recur_ms,
            gdn_gate_ms: self.spans.gdn_gate_ms,
            gdn_out_proj_ms: self.spans.gdn_out_proj_ms,
            ...
            gdn_metal_wait_ns: gdn_wait,
            gdn_metal_kernel_ns: gdn_kernel,
            gdn_metal_calls: gdn_calls,
            moe_metal_wait_ns: moe_wait,
            moe_metal_kernel_ns: moe_kernel,
            moe_metal_calls: moe_calls,
            fused_calls,
            fused_experts,
            mio_loads: mio.loads,
            mio_bytes: mio.bytes,
            ...
            features: RuntimeFeatures {
                ...
                gdn_ane: self.gdn_ane.iter().any(crate::gdn_ane::active),
                gdn_ane_fused: self.gdn_ane.iter().any(crate::gdn_ane::fused_active),
                gdn_single_copy: env_bool("QWEN_GDN_SINGLE_COPY", true),
                gdn_single_encoder: env_bool("QWEN_GDN_SINGLE_ENCODER", true),
