...
fn main() {
    let cfg = match logan_qwen4::pool::PoolConfig::from_env() {
        Some(c) => c,
        None => { println!("set LOGAN_POOL_COORDINATOR"); return; }
    };
    ...
    match &first {
        Ok(v) => println!("  batch 1 (cold connect): {:?} -> {} experts", t0.elapsed(), v.len()),
        Err(e) => println!("  batch 1 FAILED: {e}"),
    }
    for i in 2..=6 {
        let t = std::time::Instant::now();
        match logan_qwen4::pool::run_expert_batch(&cfg, &calls, d, m, "silu") {
            Ok(v) => println!("  batch {i} (reused):      {:?} -> {} experts, first={:.6}", t.elapsed(), v.len(), v[0][0]),
