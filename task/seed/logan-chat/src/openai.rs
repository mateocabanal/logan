...
use crate::protocol::minicpm5::{self, MiniCpm5TemplateOptions, ThinkingMode, ToolDefinition};
...
/// Render the MiniCPM5 prompt without consulting the Qwen prefix constant.
pub fn render_minicpm5_prompt(
    messages: &[ApiMessage],
    instructions: Option<&str>,
    thinking: bool,
    tools: Option<&Value>,
    ...
    let tool_defs = normalize_openai_tools(tools)?;
    minicpm5::render_prompt(
        &all,
...
pub fn normalize_openai_tools(tools: Option<&Value>) -> Result<Vec<ToolDefinition>, String> {
    ...
    items.iter().map(ToolDefinition::from_openai).collect()
}

/// OpenAI commonly transports function arguments as a JSON string.  Decode
/// that representation once while leaving ordinary string arguments intact.
pub fn normalize_openai_string_argument(argument: &str) -> Result<Value, String> {
    match serde_json::from_str(argument) {
        Ok(value) => Ok(value),
        Err(_) => Ok(Value::String(argument.to_string())),
    }
}

pub fn normalize_openai_argument(argument: &Value) -> Result<Value, String> {
    match argument {
        Value::String(value) => normalize_openai_string_argument(value),
