...
impl AneSurface {
    ...
    pub fn pack_bf16_transposed_f32(
        ...
        if weights_bf16.len() != want || weights_bf16.as_ptr() as usize % 2 != 0 {
            return Err(AneError::InvalidArgument(
                "BF16 pack byte size/alignment mismatch".into(),
            ));
        }
        let ok = unsafe {
            logan_ane_surface_pack_bf16_transposed_f32(
                self.raw.cast(),
                total_spatial,
                weight_offset,
                weights_bf16.as_ptr().cast(),
                in_features,
                out_features,
            )
        };
        if ok == 1 {
            ...
            Err(AneError::Surface {
                operation: "pack BF16 transpose",
                code: ok,
            })
        }
    }
    ...
    pub fn write_repeated_f32(
        ...
        let ok = unsafe {
            logan_ane_surface_write_repeated_f32(
                self.raw.cast(),
                total_spatial,
                token_spatial,
                x.as_ptr(),
