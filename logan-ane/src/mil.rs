...
/// Shape contract for the qualification-only MiniCPM5 dense FFN island.
///
/// `logical_width` is the model width and `padded_width` is the physical
/// channel count carried by the ANE tensor.  Padding is explicit rather than
/// silently inferred so callers can account for the bytes crossing the
/// boundary.  The generated program keeps the padded lanes in its result;
/// the caller must zero/ignore them according to this contract.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct FusedDenseFfnShape {
...
impl FusedDenseFfnShape {
    pub fn new(
        ...
        if logical_width == 0 || padded_width == 0 || intermediate_width == 0 || spatial == 0 {
            return Err(AneError::InvalidArgument(
                "fused dense FFN dimensions must be non-zero".into(),
            ));
        }
        if logical_width > padded_width {
            return Err(AneError::InvalidArgument(format!(
                "fused dense FFN logical width {logical_width} exceeds padded width {padded_width}"
            )));
        }
        if padded_width % 16 != 0 {
            return Err(AneError::InvalidArgument(format!(
                "fused dense FFN padded width {padded_width} must be a multiple of 16"
            )));
        }
        if spatial < 16 || spatial % 16 != 0 {
            return Err(AneError::InvalidArgument(format!(
                "fused dense FFN spatial dimension {spatial} must be >= 16 and a multiple of 16"
            )));
        }
        Ok(Self {
            logical_width,
    ...
    pub fn padded_width_for(logical_width: usize) -> Result<usize> {
        if logical_width == 0 {
            return Err(AneError::InvalidArgument(
                "fused dense FFN logical width must be non-zero".into(),
            ));
        }
        logical_width
            .checked_add(15)
            .map(|width| width / 16 * 16)
            .ok_or_else(|| AneError::InvalidArgument("fused dense FFN width overflow".into()))
    }
}

/// Generate the MiniCPM5 dense FFN qualification graph.
///
/// The graph is deliberately expressed with runtime weight inputs:
/// `x`, `rms_weight`, `gate_weight`, `up_weight`, and `down_weight`.  This
/// keeps the shape contract independently testable and avoids manufacturing a
/// fake checkpoint in a probe.  The current private ABI has rejected
/// multi-input dynamic-weight evaluation (`0x1d` on the validated M2), so this
/// function is a MIL generator only; callers must report compile/evaluate
/// failure and use their normal CPU/Metal path.
///
/// Logical input/output width is `shape.logical_width`; physical tensors carry
/// `shape.padded_width` channels.  RMSNorm, SwiGLU, down projection, and the
/// residual add all operate on the padded shape.  Padding lanes are not
/// cropped by this graph.
pub fn minicpm5_dense_ffn_fp16_f32_io(shape: FusedDenseFfnShape) -> Result<MilProgram> {
    let FusedDenseFfnShape {
        logical_width,
        padded_width,
        intermediate_width,
        spatial,
    } = shape;
    // Re-run validation for callers constructing the public struct directly.
    FusedDenseFfnShape::new(logical_width, padded_width, intermediate_width, spatial)?;

    let text = format!(
        r#"program(1.3)
[buildInfo = dict<string, string>({{{{"coremlc-component-MIL", "3510.2.1"}}, {{"coremlc-version", "3505.4.1"}}, {{"coremltools-component-milinternal", ""}}, {{"coremltools-version", "9.0"}}}})]
{{
 func main<ios18>(tensor<fp32, [1, {padded_width}, 1, {spatial}]> x, tensor<fp16, [1, {padded_width}, 1, 1]> rms_weight, tensor<fp16, [{intermediate_width}, {padded_width}, 1, 1]> gate_weight, tensor<fp16, [{intermediate_width}, {padded_width}, 1, 1]> up_weight, tensor<fp16, [{padded_width}, {intermediate_width}, 1, 1]> down_weight) {{
  string to_fp16 = const()[name = string("to_fp16"), val = string("fp16")];
  tensor<fp16, [1, {padded_width}, 1, {spatial}]> x16 = cast(dtype = to_fp16, x = x)[name = string("cast_input")];
  tensor<int32, [1]> norm_axes = const()[name = string("norm_axes"), val = tensor<int32, [1]>([1])];
  bool keep_norm = const()[name = string("keep_norm"), val = bool(true)];
  tensor<fp16, [1, 1, 1, {spatial}]> squared = mul(x = x16, y = x16)[name = string("rms_squared")];
  tensor<fp16, [1, 1, 1, {spatial}]> second = reduce_mean(axes = norm_axes, keep_dims = keep_norm, x = squared)[name = string("rms_second")];
  tensor<fp16, [1, 1, 1, {spatial}]> eps = const()[name = string("rms_epsilon"), val = tensor<fp16, [1, 1, 1, 1]>([0.00001])];
  tensor<fp16, [1, 1, 1, {spatial}]> variance = add(x = second, y = eps)[name = string("rms_variance")];
  tensor<fp16, [1, 1, 1, {spatial}]> inverse = rsqrt(x = variance)[name = string("rms_inverse")];
  tensor<fp16, [1, {padded_width}, 1, {spatial}]> normalized = mul(x = x16, y = inverse)[name = string("rms_normalized")];
  tensor<fp16, [1, {padded_width}, 1, {spatial}]> scaled = mul(x = normalized, y = rms_weight)[name = string("rms_scale")];
  ...
  tensor<fp16, [1, {intermediate_width}, 1, {spatial}]> gate = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = gate_weight, x = scaled)[name = string("gate_projection")];
  tensor<fp16, [1, {intermediate_width}, 1, {spatial}]> up = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = up_weight, x = scaled)[name = string("up_projection")];
  tensor<fp16, [1, {intermediate_width}, 1, {spatial}]> gate_sigmoid = sigmoid(x = gate)[name = string("gate_sigmoid")];
  tensor<fp16, [1, {intermediate_width}, 1, {spatial}]> swish = mul(x = gate, y = gate_sigmoid)[name = string("silu")];
  tensor<fp16, [1, {intermediate_width}, 1, {spatial}]> gated = mul(x = swish, y = up)[name = string("swiglu")];
  tensor<fp16, [1, {padded_width}, 1, {spatial}]> down = conv(dilations = dilations, groups = groups, pad = pad, pad_type = pad_type, strides = strides, weight = down_weight, x = gated)[name = string("down_projection")];
  tensor<fp16, [1, {padded_width}, 1, {spatial}]> residual = add(x = x16, y = down)[name = string("residual")];
  string to_fp32 = const()[name = string("to_fp32"), val = string("fp32")];
  tensor<fp32, [1, {padded_width}, 1, {spatial}]> y = cast(dtype = to_fp32, x = residual)[name = string("cast_output")];
 }} -> (y);
}}
"#
    );
...
/// Generic name for callers that qualify the same MiniCPM5-shaped fused FFN.
pub fn fused_dense_ffn_fp16_f32_io(shape: FusedDenseFfnShape) -> Result<MilProgram> {
    minicpm5_dense_ffn_fp16_f32_io(shape)
}
...
pub fn qwen4_mtp_input_projections_fp16_f32_io(
    ...
    parallel_dense_fp16_f32_io(hidden_size, packed_spatial, &[fc_embedding, fc_hidden])
}
...
pub fn parallel_dense_packed_dynamic_f32_io(
    ...
    for &out in out_features {
        weight_offsets.push(total_spatial);
        total_spatial = total_spatial.checked_add(out).ok_or_else(|| {
            AneError::InvalidArgument("packed dynamic f32 spatial overflow".into())
        })?;
    }
...
mod tests {
    ...
    #[test]
    fn minicpm5_dense_ffn_keeps_logical_and_padded_shape_contract() {
        let shape = FusedDenseFfnShape::new(7, 16, 32, 16).unwrap();
        let p = minicpm5_dense_ffn_fp16_f32_io(shape).unwrap();
        assert!(p.text().contains("tensor<fp32, [1, 16, 1, 16]> x"));
        assert!(p.text().contains("tensor<fp16, [1, 32, 1, 16]> gate"));
        assert!(p.text().contains("rms_second"));
        assert!(p.text().contains("swiglu"));
        assert!(p.text().contains("down_projection"));
        assert!(p.text().contains("residual"));
        assert!(p.weights().is_empty());
    }

    #[test]
    fn minicpm5_dense_ffn_validates_padding_and_spatial() {
        assert_eq!(FusedDenseFfnShape::padded_width_for(1).unwrap(), 16);
        assert_eq!(FusedDenseFfnShape::padded_width_for(7).unwrap(), 16);
        assert_eq!(FusedDenseFfnShape::padded_width_for(16).unwrap(), 16);
        assert_eq!(FusedDenseFfnShape::padded_width_for(17).unwrap(), 32);
        assert!(FusedDenseFfnShape::new(17, 16, 32, 16).is_err());
        assert!(FusedDenseFfnShape::new(7, 15, 32, 16).is_err());
        assert!(FusedDenseFfnShape::new(7, 16, 32, 15).is_err());
    }
